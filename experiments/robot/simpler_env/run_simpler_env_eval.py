...
def initialize_model(cfg: GenerateConfig):
    ...
    if cfg.use_proprio:
        proprio_projector = get_proprio_projector(
            cfg,
            model.llm_dim,
            proprio_dim=8,  # 8-dimensional proprio for SimplerEnv [x,y,z,qx,qy,qz,qw,gripper]
        )
...
def prepare_observation(obs, env, resize_size):
    ...
    if "agent" in obs and "base_pose" in obs["agent"]:
        robot_pose = obs["agent"]["base_pose"]
    else:
        # Fallback: create dummy robot state if not available
        robot_pose = np.zeros(7)  # Default 7D state
    
    # Ensure 8D state matches training format: [x,y,z,qx,qy,qz,qw,gripper]
    if len(robot_pose) == 7:
        robot_pose = np.append(robot_pose, 0.0)  # Add gripper state if missing
    elif len(robot_pose) < 7:
        # Pad to 7D EEF pose + 1D gripper = 8D total
        robot_pose = np.pad(robot_pose, (0, 8 - len(robot_pose)))
    
    # Use quaternion-based format to match data collection and training
    # Format: [x,y,z,qx,qy,qz,qw,gripper] - consistent with rlds.py collection
    eef_state = robot_pose[:7]  # First 7 elements (x,y,z,qx,qy,qz,qw)
    gripper_state = robot_pose[7:8] if len(robot_pose) >= 8 else [0.0]  # 8th element (gripper)
    
    # Combine for full 8D proprioception (no additional padding needed)
    full_state = np.concatenate([eef_state, gripper_state]).astype(np.float32)  # [8D]
    
    # Prepare observations dict
    observation = {
        "full_image": img_resized,  # OpenVLA expects "full_image" key
        "state": full_state,
    }
...
"""
# merge LoRA weights and save final checkpoint
cd /project/fhliang/projects/openvla-oft && python vla-scripts/merge_lora_weights_and_save.py --base_checkpoint "openvla/openvla-7b" --lora_finetuned_checkpoint_dir "/project/fhliang/projects/openvla-oft/checkpoints/simpler_oft/openvla-7b+simpler_env_switch_dataset+b64+lr-0.0005+lora-r32+dropout-0.0--image_aug--simpler_OFT--10000_chkpt"

# run evaluation
cd /project/fhliang/projects/openvla-oft && python experiments/robot/simpler_env/run_simpler_env_eval.py --pretrained_checkpoint "/project/fhliang/projects/openvla-oft/checkpoints/simpler_oft/openvla-7b+simpler_env_switch_dataset+b64+lr-0.0005+lora-r32+dropout-0.0--image_aug--simpler_OFT--10000_chkpt" --task_suite_name "all" --num_trials_per_task 10 --use_l1_regression True --use_proprio False --lora_rank 32 --seed 42 --run_id_note "simpler_oft_eval"
"""

#cd /project/fhliang/projects/openvla-oft && python experiments/robot/simpler_env/run_simpler_env_eval.py --pretrained_checkpoint "/project/fhliang/projects/openvla-oft/checkpoints/simpler_oft/openvla-7b+simpler_env_switch_dataset+b64+lr-0.0005+lora-r32+dropout-0.0--image_aug--simpler_OFT--10000_chkpt" --task_suite_name "all" --num_trials_per_task 10 --use_l1_regression True --use_proprio True --lora_rank 32 --seed 42 --run_id_note "simpler_oft_eval_with_proprio"

#cd /project/fhliang/projects/openvla-oft && python experiments/robot/simpler_env/run_simpler_env_eval.py --pretrained_checkpoint "openvla/openvla-7b" --task_suite_name all --num_trials_per_task 10 --use_l1_regression True --use_proprio True