...
def run_task(
    ...
    # Get holdout prompt pool for random sampling
    holdout_prompt_pool = None
    if holdout_prompts:
        normalized_task = original_task_description.lower().strip()
        if normalized_task in holdout_prompts and holdout_prompts[normalized_task]:
            holdout_prompt_pool = holdout_prompts[normalized_task]

    log_message(f"\n{'='*80}", log_file)
    log_message(f"TASK {task_id}: {original_task_description}", log_file)
    if holdout_prompt_pool:
        log_message(f"Randomly sampling from {len(holdout_prompt_pool)} holdout prompts each episode", log_file)
    log_message(f"Episodes: {cfg.num_trials_per_task}", log_file)
    ...
    episode_idx = 0
    with tqdm.tqdm(total=cfg.num_trials_per_task, desc=f"Task {task_id}") as pbar:
        while task_episodes < cfg.num_trials_per_task:
            ...
            # Randomly sample a holdout prompt, or use original
            if holdout_prompt_pool:
                task_description = random.choice(holdout_prompt_pool)
            else:
                task_description = original_task_description

            log_message(f"Starting episode {task_episodes + 1}, prompt: {task_description}", log_file)
            ...
            video_id = f"{task_id}_{task_episodes}"
            save_rollout_video(
                replay_images, video_id, success=success,
                task_description=f"[Task {task_id}] {task_description}", log_file=log_file, run_id=run_id
            )
    ...
    log_message(f"\n{'='*80}", log_file)
    log_message(f"TASK {task_id}: {task_successes}/{task_episodes} ({task_success_rate*100:.1f}%)", log_file)
    log_message(f"TOTAL: {total_successes}/{total_episodes} ({total_success_rate*100:.1f}%)", log_file)
    log_message(f"{'='*80}", log_file)
...
# one liner QD eval:
# CUDA_VISIBLE_DEVICES=1 python experiments/robot/libero/run_libero_eval.py --pretrained_checkpoint ./checkpoints/libero_spatial_oft_QD64_Repeat/openvla-7b+libero_spatial_no_noops+b32+lr-0.0005+lora-r32+dropout-0.0--image_aug--liberoSpatial_oft_QD64_Repeat--5000_chkpt --task_suite_name libero_spatial --center_crop True --num_trials_per_task 50 --use_l1_regression True --use_diffusion False --use_film False --num_images_in_input 2 --use_proprio True --lora_rank 32 --use_wandb True --wandb_entity freddieliang-usc --wandb_project openvla_repl --run_id_note liberoSpatial_oft_QD64_eval_QD_holdout --holdout_prompts_yaml ./QD_eval_tasks.yaml

#ERT64 on QD_NR_P:
# CUDA_VISIBLE_DEVICES=1 python experiments/robot/libero/run_libero_eval.py --pretrained_checkpoint ./checkpoints/libero_spatial_oft_ERT64/openvla-7b+libero_spatial_no_noops+b64+lr-0.0005+lora-r32+dropout-0.0--image_aug--liberoSpatial_oft_ERT64--5000_chkpt --task_suite_name libero_spatial --center_crop True --num_trials_per_task 50 --use_l1_regression True --use_diffusion False --use_film False --num_images_in_input 2 --use_proprio True --lora_rank 32 --use_wandb True --wandb_entity freddieliang-usc --wandb_project openvla_repl --run_id_note ERT64_EVAL_on_QD_NR_P --holdout_prompts_yaml /home/freddie/project/openvla-oft/QD_eval_tasks_preprocess_no_repeat.yaml

#QD_NR_P eval on ERT64:
# CUDA_VISIBLE_DEVICES=1 python experiments/robot/libero/run_libero_eval.py --pretrained_checkpoint /home/freddie/project/openvla-oft/checkpoints/libero_spatial_oft_QD64_Preprocess_NR/openvla-7b+libero_spatial_no_noops+b32+lr-0.0005+lora-r32+dropout-0.0--image_aug--liberoSpatial_oft_QD64_Repeat--5000_chkpt --task_suite_name libero_spatial --center_crop True --num_trials_per_task 50 --use_l1_regression True --use_diffusion False --use_film False --num_images_in_input 2 --use_proprio True --lora_rank 32 --use_wandb True --wandb_entity freddieliang-usc --wandb_project openvla_repl --run_id_note EVAL_QD_NR_P_on_ERT --holdout_prompts_yaml /home/freddie/project/openvla-oft/ERT_eval_tasks.yaml

#QD_NR on QD eval:
# CUDA_VISIBLE_DEVICES=0 python experiments/robot/libero/run_libero_eval.py --pretrained_checkpoint /home/freddie/project/openvla-oft/checkpoints/libero_spatial_oft_QD_NR/openvla-7b+libero_spatial_no_noops+b64+lr-0.0005+lora-r32+dropout-0.0--image_aug--liberoSpatial_oft_QD_NR_QD_VLM_no_repeat--5000_chkpt --task_suite_name libero_spatial --center_crop True --num_trials_per_task 50 --use_l1_regression True --use_diffusion False --use_film False --num_images_in_input 2 --use_proprio True --lora_rank 32 --use_wandb True --wandb_entity freddieliang-usc --wandb_project openvla_repl --run_id_note liberoSpatial_QD_NR_eval_QD_holdout --holdout_prompts_yaml /home/freddie/project/openvla-oft/QD_eval_tasks_no_repeat.yaml
...
#one liner QD eval:
# CUDA_VISIBLE_DEVICES=0 python experiments/robot/libero/run_libero_eval.py --pretrained_checkpoint moojink/openvla-7b-oft-finetuned-libero-spatial --task_suite_name libero_spatial --center_crop True --num_trials_per_task 50 --use_l1_regression True --use_diffusion False --use_film False --num_images_in_input 2 --use_proprio True --lora_rank 32 --use_wandb True --wandb_entity freddieliang-usc --wandb_project openvla_repl --run_id_note liberoSpatial_moojink_eval_QD_holdout --holdout_prompts_yaml ./QD_eval_tasks.yaml

#eval on QD_NR:
# CUDA_VISIBLE_DEVICES=1 python experiments/robot/libero/run_libero_eval.py --pretrained_checkpoint moojink/openvla-7b-oft-finetuned-libero-spatial --task_suite_name libero_spatial --center_crop True --num_trials_per_task 50 --use_l1_regression True --use_diffusion False --use_film False --num_images_in_input 2 --use_proprio True --lora_rank 32 --use_wandb True --wandb_entity freddieliang-usc --wandb_project openvla_repl --run_id_note liberoSpatial_moojink_eval_QD_NR_holdout --holdout_prompts_yaml /home/freddie/project/openvla-oft/QD_eval_tasks_no_repeat.yaml